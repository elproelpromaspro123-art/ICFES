...
export default function SimulacroMatematicasClient() {
  ...
  useEffect(() => {
    ...
    for (const mode of modes) {
      nextMap[mode.count] = safeParseJSON<SimulacroProgress>(
        localStorage.getItem(makeProgressKey(mode.count, randomize))
      );
    }
  ...
  const clearProgress = (count: number) => {
    if (typeof window === "undefined") return;
    localStorage.removeItem(makeProgressKey(count, randomize));
